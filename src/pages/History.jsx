...
export default function History({ session }) {
  ...
  const [viewUserId, setViewUserId] = useState(session.user.id)
  ...
  useEffect(() => {
    ...
    async function load() {
      const [{ data: ws }, { data: nl }] = await Promise.all([
        supabase.from('workout_sessions').select('*').eq('user_id', viewUserId).order('date', { ascending: false }),
        supabase.from('nutrition_logs').select('*').eq('user_id', viewUserId).order('date', { ascending: false }),
      ])
      setWorkouts(ws || [])
      setNutritionLogs(nl || [])
      setLoading(false)
  ...
  }, [tab, selectedExercise, viewUserId])
  ...
  async function loadChartData(exercise) {
    ...
    const data = Object.entries(byDate)
      .sort(([a], [b]) => (a < b ? -1 : 1))
      .map(([date, { weight, reps }]) => ({ date, label: fmtShort(date), weight, reps }))
  ...
  const viewingName = profiles.find(p => p.id === viewUserId)?.display_name
  ...
  return (
    <div className="space-y-5">
      ...
      {/* User picker (only shown when there are other users) */}
      {profiles.length > 1 && (
        <div>
          <label className="block text-xs font-medium text-gray-500 mb-1.5">Viewing</label>
          <select
            value={viewUserId}
          ...
          >
            {profiles.map(p => (
              <option key={p.id} value={p.id}>
                {p.id === session.user.id ? `${p.display_name} (you)` : p.display_name}
              </option>
            ))}
          </select>
        </div>
